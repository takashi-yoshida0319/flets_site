(function($) {
    ...
    view: {
      ...
      FormView: function(obj) {
        ...
        this.initialize = function(obj) {
          ...
          this.$el.find('.boxdd').bind('click', $.proxy(this.onModelChange, this));
        ...
        this.renderSelectBox = function(model, key, value) {
          ...
          this.$el.find('select').each($.proxy(function(i, item) {
            if (select_index == i && value != "") {
              ...
              $select.next('.boxdd').html('');
              $.each(data, function(i, item) {
                $('<option>' + $.trim(item) + '</option>').attr({'value': i}).appendTo($select);
                var pref = $select.attr('id') + '_' + i;
                var html_div = $('<div/>').attr({'class':'panel_radio'});
                var html_input = $('<input/>').attr({'value':i, 'id':pref, 'type':'radio', 'name':'input_'+$select.attr('id'), 'tabindex':'0'});
                var html_label = $('<label/>').attr({'for':pref});
                var html_span = $('<span/>').attr({'class':'ic_radio'});
                var html_p = $('<p>'+$.trim(item)+'</p>');
                var html_img = $('<img/>').attr({'src':'/user/support/solve/notconnect/check_03/img/'+pref+'.png'});
                if ($select.attr('id') != 'c4' && $select.attr('id') != 'c5') {
                  html_span.appendTo(html_label);
                  html_img.appendTo(html_label);
                  html_p.appendTo(html_label);
                  html_input.appendTo(html_div);
                  html_label.appendTo(html_div);
                  html_div.appendTo($select.next('.boxdd'));
                }
              });
              $(item).parents('.whitebox').removeClass('select-none');
            } else if (select_index < i || (select_index == i && value == "")) {
              $(item).html('<option value="">お選びください</option>');
              $(item).parents('.whitebox').addClass('select-none');
            }
            if (select_index == 3 || select_index == 4 || select_index == 5) {
              $(item).parents('.whitebox').removeClass('select-none');
            }
          }, this));
        ...
        this.onModelChange = function(e) {
          ...
          var boxdd = this.$el.find('.boxdd');
          if ($target.hasClass('boxdd')) {
            var key = $target.find("input").attr("name").replace('input_', '');
            var value = $target.find("input[name=\"input_"+key+"\"]:checked").val();
            var index = boxdd.index($target);
            if (this.$el.find('select').eq(index).val() != "" || value != undefined) {
              if (this.$el.find('select').eq(index).val() != value) {
                var position = document.getElementsByClassName('whitebox')[index + 1].offsetTop;
                $('body,html').animate({"scrollTop":position}, 400, 'swing');
              }
            }
            this.$el.find('select').eq(index).val(value);
          } else {
            var key = $target.attr('name');
            var value = $target.find('option:selected').val();
            var index = this.$el.find('select').index($target);
            boxdd.eq(index).find("input[name=\"input_"+key+"\"]").val([value]);
          }
          this.model.set(key, value);
          // チェンジの発生した以降の内容を空にする
          this.$el.find('select').each($.proxy(function(i, item) {
            if (index < i) {
              boxdd.eq(i).find("input:radio").prop('checked',false);
              $(item).val("");
              this.model.set(this.prefix + parseFloat(i + 1), "", false);
            }
          }, this));
        ...
        this.onModelClear = function(e) {
          ...
          $selects.each($.proxy(function(i, item) {
            if ($(item).attr('name') == "c1") {
              return;
            }
            $(item).val("");
            $form.find('.boxdd').eq(i).find("input:radio").prop('checked',false);
      ...
      FormLampView: function(obj) {
        ...
        this.render = function() {
          var select = this.choice[this.type];console.log(select)
          $.each(select, $.proxy(function(i, item) {
            $('#checktable_0' + parseFloat(i + 1)).html();
          }, this));
          $.each(select, $.proxy(function(i, item) {
            this.renderTableRow(item, $('#checktable_0' + parseFloat(i + 1)));
            if (undefined == $('#checktable_0' + parseFloat(i + 1))[0]) {
              $('.lamp-select__group').eq(0).addClass('view-none');
            }
          }, this));
          // ADD 20180430
        ...
        this.renderTableRow = function(rows, $tableObj) {
          $.each(rows, $.proxy(function(i, item) {
            var $row = $('<li class="checktable__item"/>').append('<div class="checktable__item-label"/>').append('<div class="checktable__item-select"/>');
        ...
        this.onModelClear = function(e) {
          this.setNormalValue(this.getNormalValue());
          $.cookie(COOKIE_KEY, this.model.getQueryString(), { path: '/' });
          var position = document.getElementById('step1-q1').parentNode.offsetTop;
          $('body,html').animate({"scrollTop":position}, 400, 'swing');
        };
      ...
      FormConfirmView: function(obj) {
        ...
        this.render = function() {
          ...
          $.each(select, $.proxy(function(i, item) {
            ...
            if (undefined == $('#confirmTable_0' + parseFloat(i + 1)).children()[0]) {
              $('.lamp-select__group').eq(0).addClass('view-none');
            }
          }, this));
        };
        ...
        this.renderTableRow = function(rows, $tableObj) {
          $.each(rows, $.proxy(function(i, item) {
            var $row = $('<li class="confirmtable__item"/>').append('<div class="confirmtable__item-label"/>').append('<div class="confirmtable__item-select"/>');
            // ADD 2018
  ...
  $(function() {
    ...
    page('/user/support/solve/notconnect/result_error_check_03/(index.html)?',router.enquete, router.complete);
    page('/user/support/solve/notconnect/result_ok_check_03/(index.html)?',router.enquete, router.complete);
