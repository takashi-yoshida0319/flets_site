...
(function ($) {
	...
	$(document).ready(function () {
		...
		$("#kaizen_section__cancel").attr({"style":"height: auto;"});
		...
		$(".kaizen_text__index").attr({"style": "font: normal normal bold 18px/21px Noto Sans JP;"});
		...
		/*ステップ1 */
		$(".slick-list").attr("style","height:538px;");
		...
		$(".kaizen_section__suggest__1 > h2").html("コラボ光へのご変更は、<span>転用</span>のお手続きをお願いします。");
		...
		$(".mb40 > .c-txt").after(
			...
			"<img src=\"https://cdn.kaizenplatform.net/v2/attachments/000/375/989/017e65844e8d5d70d23487f64dc3a203a186fb3c.png\" style=\"display: inline-block;width: 36px;height: 36px;margin: auto;margin-top: 5px;\">" +
			"<p style=\"display: inline-block;margin-left: 15px;text-align: left;font: normal normal bold 20px/32px Noto Sans JP;margin-top: -10px;\">" +
			"コラボ光のご解約はこちらから行うことができません。<br>各光コラボレーション事業者さまへお申し込みください。</p>" +
			"</div>"
		  );
		$(".kaizen_section__suggest__1").attr({"style":"margin-top: 30px;"});
		$(".kz_step1_content_style").attr({"style":"height: 100%;width: 77%;border: 5px solid #FC5255;margin: auto;margin-top: 50px;margin-bottom:110px;padding: 30px;"});
		$(".kz_step1_content_style > img").attr({"style":"display: inline-block;width: 36px;height: 36px;margin: auto;margin-top: 5px;margin-bottom: 5px;"});

		/*ステップ2 */
		$(".kaizen_section__suggest__2 > h2").html("NTT西日本エリア内でお引越しの場合<br><span>継続</span>してご利用いただけます。")
		$(".title > span").attr({"class":"underline"});
		...
		$(".wrap_text > img").attr({"style":"top: 618px;left: 533px;width: 209px;height: 192px;margin: auto;"});
		...
		/*ステップ4 */
		$(".kaizen_section__suggest__4 > h2").html("設定の見直しやプラン変更で改善できる可能性があります");
		...
		$("._sp_mb40 > .btn-panel_link > div:eq(0) > p:eq(0)").after(
			"<p style=\"font-size: 25px;text-align: center;margin-top: 10px;margin-auto;\">" +
			  ...
			  "<span style=\"color: #1973B9;\">最大概ね<span class=\"temp-class5\" style=\";font: var(--unnamed-font-style-normal) normal 900 32.4px/26px var(--unnamed-font-family-noto-sans-jp);text-align: center;font: normal normal 900 32px/26px Noto Sans JP;letter-spacing: 1.62px;color: #1973B9;\">10</span>ギガ</span>の世界へ!" +
		...
		$(".pt25").remove();
		$("._sp_mb40 > .btn-panel_link > div:eq(0) > p:eq(1)").after("<p style=\"margin-top: 10px;margin-bottom: 10px;margin: auto;text-align: center;\">ダウンロード時間、<span class=\"underline\">最大速度約10倍!</span> (理論値)</p>");
		...
		$(".kaizen_section__suggest__4 .kaizen_block__discountBtn").after("<ul class=\"c-note_list\"><li>規格速度がより高速なタイプでも、利用環境等により実効速度が向上しない場合があります。</li><li>「セキュリティ対策ツール」などの標準機能や一部のオプションサービスがお使いいただけません。</li><li>サービス提供エリアのすべての住所でサービス提供を保証するものではありません。エリアについては、[<a href=\"/cart/\" class=\"link_icn blank\" target=\"_blank\" tabindex=\"0\">https://flets-w.com/cart/</a>]をご確認ください。</li></ul>");
		$(".kaizen_section__suggest__4 .c-note_list:eq(1) li:eq(2)").remove();

		$("._sp_mb40 > .btn-panel_link").attr({"style":"height: 100%;width: 100%;"});
		$("._sp_mt10 > ._sp_mb25 > a:eq(0)").attr({"style":"height: 100%;"});
		$("._body > .mt20").attr({"style":"margin-bottom: 20px!important;font-weight: bold;"});
		$("._sp_mt10 > ._sp_mb25 > a:eq(0) > div:eq(0) > img:eq(0)").attr({"style":"height: 98px;margin: auto;margin-top: 25px;"});
		...
		$("._sp_mb40 > .btn-panel_link > div:eq(0)").attr({"style":"margin-bottom: 30px;margin-top: -5px;"});
